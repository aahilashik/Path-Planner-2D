...
bool GreedyBestFirstSearch::setPlannedPath(Node *nodes, Node *nodeStart, Node *nodeGoal) {
    ...
    for (int x = 0; x < this->mapWidth; x++)
        for (int y = 0; y < this->mapHeight; y++) {
            ...
            */
            ...
            nodes[x * this->mapWidth + y].costMap["Heuristic"] = INFINITY;
    ...
    // Set Heuristic Cost of current node as 0.0
    nodeCurrent->costMap["Heuristic"] = 0.0f;
    ...
    while (!openNodesList.empty() && nodeCurrent != nodeGoal) {
        iteration++;
        openNodesList.sort([](Node* lhs, Node* rhs){ return lhs->costMap["Heuristic"] < rhs->costMap["Heuristic"]; } );
        ...
        for (auto nodeNeighbour : nodeCurrent->nodeNeighbours) {
            ...
            float fPossiblyLowerGoal = nodeCurrent->costMap["Heuristic"] + heuristic(nodeCurrent, nodeNeighbour);

            if (fPossiblyLowerGoal < nodeNeighbour->costMap["Heuristic"]) {
                nodeNeighbour->parents["Parent"] = nodeCurrent;
                nodeNeighbour->costMap["Heuristic"] = heuristic(nodeNeighbour, nodeGoal);
            }
        }	
